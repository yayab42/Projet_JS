$(document).ready(function () {
  ...
  function loadfeed() {
    ...
    $.ajax({
      url: "https://www.cheapshark.com/api/1.0/deals?&upperPrice=10",
      method: "GET",
      dataType: "json",
  ...
  function form() {
    ...
    name='name' id='name' required></div><div><label for='note'>Note ( entre 0 et 100 ):</label><input type='number'\
     ...
     <button type='button' value='Envoyer' id='submit'>Envoyer</button></form></div>"
    ).appendTo(".formbtn");
  }
  ...
  let iciForm = document.querySelector(".formbtn");
  console.log(iciForm);
  iciForm.innerHTML="<button id='rld'><p>Ajouter une review</p></button>";
  ...
  $(".rld").prepend("<button><p>Recharger</p></button>");
